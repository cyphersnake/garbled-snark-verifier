...
pub fn optimized_sub_generic(
    ...
    for i in 0..len {
        circuit.add_wire(Wire::new_rc());
        if i > 0 {
            let subtract_bit = Wire::new_rc();
            ...
            let new_want_or0 = Wire::new_rc();
            let new_want_or1 = Wire::new_rc();
            let new_want = Wire::new_rc();
    ...
    if check_bound {
        let bound_check_wire = Wire::new_rc();
